                        ...
                        <div class="panel-body">
                            ...
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                <c:forEach var="item" items="${list}">
                                    <tr>
                                        <td class="text-center order-no"><a href="/check-out.jsp?orderNo=<c:out
                                                value="${item.orderNo}"/>">
                                        ...
                                        <td class="text-center">
                                            <c:out value="${item.returnDateInActual}"/>
                                        </td>
                                        <td class="text-center">
                                            <c:out value="${item.totalConsume}"/>
                                        </td>
                                        <td class="text-center">
                                            <c:out value="${item.violateDate}"/>
                                        </td>
                                        <td class="text-center yaj">
                                            <c:out value="${item.violateDeposit}"/>
                                        </td>
                                        <td class="text-center kouk">
                                            <c:out value="${item.violateDeductions }"/>
                                        </td>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-8 col-md-offset-1">
                        <div class="page-header"><b>订单号</b><span id="orderNo"></span>
                        </div>
                        <div class="row">
                            <div class="col-md-4">
                                <b>违章押金:</b> <b id="yaj"></b>
...
<script>
    ...
    $('#myModal').on('show.bs.modal', function (event) {
        ...
        modal.find('#tuik').text(parseFloat(yaj) - parseFloat(kouk))

    });

    $("#drawback").click(function () {
        var modal = $('.modal');
        var orderNo = modal.find('#orderNo').text().trim();
        $.ajax({
            type: "POST",
            url: "/Order/Finish",
            data: JSON.stringify({
            ...
            success: function (dataVal) {
                ...
                if (dataVal.RESULT_KEY == 1) {
                    alert("操作成功");
                    $('#myModal').modal('hide')
                    window.location.href = "/Order/Query/orderStatus?orderStatus=4"
                } else {
                    alert("操作失败");
                }
            },
