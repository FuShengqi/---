...
public class OrderServiceImpl implements OrderService {
    ...
    public int addRentOrder(HashMap infoMap) {
        ...
        recordCreator="0";
//        recordCreator = stuffMapper.queryIDByStuffNo(recordCreator);
        recordCreator = "90a205071316495e89bd27d92176fe3f";
        infoMap.put("recordCreator",recordCreator);
    ...
    * */
    ...
    public HashMap beforeBackCar(HashMap infoMap) {
        ...
        mapDate.put("id",id);
        mapDate.put("returnDateInActual",infoMap.get("returnDateInActual"));
        System.out.println("mapDate："+mapDate);
        orderMapper.updateReturnDateInActual(mapDate);

        System.out.println("付款订单Id："+id);
        String  carId=orderMapper.queryCarIdById(id);
         id =carId;
        System.out.println("Orderserce: beofreback" + infoMap);
...
//            System.out.println("customerName"+customerName);
           ...
           // Date orderStartDate=  new Date((long)orderInfoList.get(i).getOrderStartDate()*1000);
            ...
            String returnDateInPlanString = "";
            if (orderInfoList.get(i).getReturnDateInPlan() != null) {
                returnDateInPlanString = simpleDateFormat.format(orderInfoList.get(i).getReturnDateInPlan());
            }
            map.put("returnDateInActual",dataString1);
//            System.out.println("开始查询订单Date"+dataString1);
            map.put("returnDateInPlan", returnDateInPlanString);
            map.put("carNo",carNo);
