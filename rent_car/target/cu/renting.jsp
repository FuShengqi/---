                                ...
                                <c:forEach var="item" items="${list}">
                                    <tr class="odd gradeX">
                                        ...
                                        <td class="text-center">
                                            <a href="/check-out.jsp?orderNo=<c:out
                                                value="${item.orderNo}"/>&carNo=<c:out
                                                value="${item.carNo}"/>"><c:out value="${item.orderNo}"/></a>
                                        </td>
                                        <td class="text-center carNo"><c:out value="${item.carNo}"/></td>
                                        <td class="text-center clientName"><c:out value="${item.customerName}"/></td>
                                        <td class="text-center startTime">
                                            <c:out value="${item.orderStartDate}"/>
                                        </td>
                                        <td class="text-center">
                                            <c:out value="${item.returnDateInPlan}"/>
                                        </td>
                                        <td class="text-center">
                                            <c:out value="${item.rentDeposit}"/>
                                        </td>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">订单<span id="modalOrderNo"></span>续租</h4>
            </div>
...
<script>
    $('#myModal').on('show.bs.modal', function (event) {
        ...
        console.log(carNo);
        var modal = $(this);
        modal.find('#modalOrderNo').text(orderNo);
...
<script>
    $('#xuzu').click(function () {
//        console.log($(this))
        console.log($('#myModal').find('#modalOrderNo').text());
        console.log($('#myModal').find('#user_date').val())
        $.ajax({
            type: "POST",
            url: "/RenewalOrder/Add",
            data: JSON.stringify({
                orderNo: $('#myModal').find('#modalOrderNo').text(),
                renewalEndDate: $('#myModal').find('#user_date').val(),
                recordCreator: "ADMIN"
            }),
            async: false,
